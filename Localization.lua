...
if LOCALE == "esES" or LOCALE == "esMX" then -- official translation (thanks to twitter @RomanValoppi )
	...
	L["Hello!"] = "¡Hola amigo! El complemento SlowFaller ahora está habilitado en una clase con una habilidad de caída lenta. Salta dos veces para activar: " .. SpellNames.spells .. "\nNo funciona en combate ni con montura."
...
if LOCALE == "deDE" then
	...
	L["Hello!"] = "Hallo Freund! Das SlowFaller-Addon ist jetzt für eine Klasse mit der Fähigkeit „Slow Fall“ aktiviert. Führt einen Doppelsprung aus: " .. SpellNames.spells .. "\nFunktioniert nicht im Kampf oder auf einem Pferd."
...
if LOCALE == "frFR" then -- official translation (thanks to twitter @Solanya_ )
	...
	L["Hello!"] = "Salut l'ami! L'addon SlowFaller est maintenant activé sur une classe avec une capacité de chute lente. Faites un double saut pour activer: " .. SpellNames.spells .. "\nNe fonctionne pas en combat ou à cheval."
...
if LOCALE == "ptBR" then
	...
	L["Hello!"] = "Olá amiga! O complemento SlowFaller agora está habilitado em uma classe com uma habilidade de queda lenta. Dê um salto duplo para ativar: " .. SpellNames.spells .. "\nNão funciona em combate ou montado."
...
if LOCALE == "ruRU" then
	...
	L["Hello!"] = "Привет друг! Надстройка двойной-прыжок-замедленное-падение теперь включена для класса со способностью медленного падения. Двойной прыжок для активации: " .. SpellNames.spells .. "\nНе работает в бою или верхом."
...
if LOCALE == "koKR" then
	...
	L["Hello!"] = "안녕 친구! 이중-도약저속-낙하 애드온은 이제 느린 낙하 능력이 있는 클래스에서 활성화됩니다. 두 번 점프하여 활성화: " .. SpellNames.spells .. "\n전투 중이나 탈것에 탄 상태에서는 작동하지 않습니다."
...
if LOCALE == "zhCN" then
	...
	L["Hello!"] = "你好朋友！ 二段跳缓落术 插件现在在具有缓慢下降能力的类上启用。 双跳激活： " .. SpellNames.spells .. "\n在战斗或骑乘时不起作用。"
